...
class RemarkableBackup:
    ...
    def scan_files(self, remote_dir):
        ...
        return [
            file.filename
            for file in self.sftp.listdir_iter(remote_dir)
    ...
    def download_files(self, remote_dir, temp_destination):
        ...
        """
        return [
            self.sftp.get(remote_dir + file, temp_destination + file)
    ...
    def scan_directories(self, remote_dir):
        ...
        return [
            file_indir
            for file_indir in self.sftp.listdir_iter(remote_dir)
    ...
    def create_directories(self, remote_dir):
        ...
        for directory in self.scan_directories(remote_dir):
            ...
            try:
                ...
                print(
                    e, directory.filename, "Backup subdir already exists moving on...."
                )
            self.download_files(
                remote_dir + directory.filename + "/",
    ...
    def backup_compress(self):
        ...
        """
        with tarfile.open(
            self.destination + "remarkable_2_" + time.strftime("%m%d%Y") + ".tar.gz",
            "w:gz",
        ) as tar:
